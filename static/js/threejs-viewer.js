          ...
          if (progress.total) {
            ...
            if (loadingDiv) {
              loadingDiv.innerHTML = `
                ...
                <p style="font-size: 0.8rem; color: #999;">Large files may take up to 10 minutes...</p>
              `;
            }
          } else {
            ...
            const loadingDiv = container.querySelector('.viewer-loading');
            if (loadingDiv) {
              const elapsed = Math.floor((Date.now() - (loadingDiv.dataset.startTime || Date.now())) / 1000);
              loadingDiv.dataset.startTime = loadingDiv.dataset.startTime || Date.now();
              
              loadingDiv.innerHTML = `
                <i class="fas fa-spinner fa-spin" style="font-size: 2em; margin-bottom: 10px;"></i>
                <p>Loading 3D model...</p>
                <p style="font-size: 0.9rem; color: #666;">Processing large file... (${elapsed}s)</p>
                <p style="font-size: 0.8rem; color: #999;">This may take up to 10 minutes</p>
              `;
            }
          }
        },
